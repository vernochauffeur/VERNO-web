...
"hampton": 148, "sandringham": 150,"mentone": 155, "cheltenham": 155,
// South-east outer
...
// Returns true if the normalised address contains the group keyword as a whole word.// Prevents "hampton" matching inside "north hampton" or similar accidents.
...
// nearby -> avg_anchor * 0.40// same zone -> min_anchor * 0.50
...
const airFixed = airportFixedFare(from, to);if (airFixed !== null) {
...
- Fallback: curated Melbourne suggestions shown when API is notconfigured (demo mode).
--------------------------------------------------------------- */
...
// Regional{ main: "Mornington Peninsula", sub: "Mornington VIC 3931" },
...
// -- Bootstrap: try Google Places; fall back to local list --useEffect(() => {
...
const q = value.toLowerCase();const res = DEMO_SUGGESTIONS.filter(s =>
...
NAV
================================================ */
function Nav() {const [solid, setSolid] = useState(false);
...
Airport transfer? Set Melbourne Airport as destination</button>
<AddressField
id="bp-from"
label="Pickup"
...
<button className="btn-whatsapp" onClick={handleWA}><WAIcon s={18}/> Confirm Booking via WhatsApp
</button>
...
<div className="wrap">
<div className="rv"><div className="s-label">Services</div></div><h2 className="s-h rv d1">Every journey,<br/><em>handled.</em></h2>
...
================================================ */
function Why() {
const ref = useRef(null); useReveal(ref);return (
...
{name:"Geelong & Surf Coast",time:"~70 min airport", desc:"Geelong CBD, Torquay, Barwon H{name:"Greater Melbourne", time:"All suburbs", desc:"Essendon, Brighton, Hawthorn,
];
return (
...
"Full privacy glass and wireless device charging","Zero emissions - fully charged for every journey",
...
const ref = useRef(null); useReveal(ref);
return (
<section className="sec" ref={ref}><div className="wrap">
...
.hero-label{font-size:.68rem;font-weight:400;letter-spacing:.18em;text-transform:uppercase;co.hero-h1{font-family:var(--serif);font-size:clamp(2.8rem,6.5vw,6rem);font-weight:400;line-hei
...
.fare-trust{display:flex;flex-wrap:wrap;gap:.5rem 1.5rem;margin-top:1rem;padding-top:1rem;bor.fare-trust-item{font-size:.68rem;font-weight:300;color:rgba(255,255,255,.35);letter-spacing:
...
/* AREAS */.areas-list{display:grid;grid-template-columns:repeat(4,1fr);gap:0;border-top:1px solid var(-
...
/* TESTIMONIALS */
.testi-row{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--grey2);m.testi{background:var(--white);padding:2.8rem 2.4rem 3.2rem;transition:background .2s}.testi:
...
.proc-track{grid-template-columns:1fr}.proc-step{border-right:none;border-bottom:1px solid rgba(255,255,255,.08);padding:2rem 0!i
