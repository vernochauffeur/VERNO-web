...
// ─── HERO — yeniden düzenlendi ───────────────────────────────────────────────
function Hero() {
const wa = buildWhatsAppLink({ from: “”, to: “”, fare: null });
return (
<section className="hero">
{/* Koyu gradient overlay — arka planda şehir fotoğrafı bekleniyor */}
...
```
  <div className="hero-content">
    ...
    {/* Ana başlık: daha küçük, okunabilir, rafine */}
    <h1 className="hero-h1">
      Arrive in<br />
      <em>complete confidence.</em>
    </h1>

    {/* Ne iş yaptığımızı anlatan 3 kart */}
    <div className="hero-services">
      <div className="hero-svc-card">
        <span className="hero-svc-icon">✈</span>
...
.nav-links a,.nav-btn{font-size:.75rem;text-transform:uppercase;letter-spacing:.1em;color:rgba(255,255,255,.6)}
.nav.solid .nav-links a,.nav.solid .nav-btn{color:#555}
.nav-btn{border:1px solid rgba(255,255,255,.2);padding:.55rem 1.2rem}
.nav.solid .nav-btn{border-color:#ddd}
.nav-right{display:flex;align-items:center}
.hamburger,.mob-drawer{display:none}

/* ── HERO — yeniden tasarlandı ── */
.hero{
position:relative;
min-height:100vh;
/* Arka plan: Melbourne şehir görseli + koyu gradient katmanı */
background:
linear-gradient(
160deg,
rgba(10,10,10,.92) 0%,
...
url(‘https://images.unsplash.com/photo-1514395462421-da574054106e?w=1800&q=80’) center/cover no-repeat;
display:flex;
...
max-width:1200px;
margin:0 auto;
padding:140px 5vw 7vw;
}

/* Etiket */
.hero-label,.s-label,.moments-eyebrow{
font-size:.68rem;
font-weight:500;
letter-spacing:.2em;
...
/* Ana başlık — Cormorant Garamond, daha okunabilir boyut */
.hero-h1{
...
border-top:1px solid rgba(158,138,106,.3);
backdrop-filter:blur(8px);
}
.hero-svc-card:hover{background:rgba(158,138,106,.12)}
.hero-svc-icon{display:block;font-size:1rem;margin-bottom:.5rem;opacity:.7}
...
.btn-o{border:1px solid #e5e5e5;color:#555}

/* ── TRUST STRIP ── */
.trust-strip{background:#111;padding:.85rem 5vw;border-top:1px solid rgba(255,255,255,.06)}
.trust-strip-inner{max-width:1200px;margin:auto;display:flex;gap:2rem;flex-wrap:wrap}
.trust-item{display:flex;gap:.5rem;align-items:center;color:rgba(255,255,255,.4);font-size:.7rem;letter-spacing:.08em;text-transform:uppercase}
.trust-item-dot{width:3px;height:3px;border-radius:50%;background:var(–gold)}

/* ── BOOKING PANEL ── */
.booking-panel{padding:7rem 5vw}
.booking-panel-inner{max-width:1200px;margin:auto;display:grid;grid-template-columns:1fr 1.5fr;gap:7rem}
.booking-panel-headline,.s-h{font-family:var(–serif);font-size:clamp(2rem,4vw,3.4rem);font-weight:300;line-height:1.1;margin-bottom:2rem}
.booking-panel-headline{font-size:clamp(1.8rem,2.5vw,2.4rem)}
.booking-panel-headline em,.s-h em,.fleet-text-title em,.closer-h em{color:var(–gold);font-style:italic}
...
.fare-price{font-family:var(–serif);font-size:4rem;line-height:1;font-weight:300}
...
.btn-whatsapp{width:100%;display:flex;justify-content:center;align-items:center;gap:.6rem;background:var(–wa);color:#fff;padding:1.1rem;margin-top:1.5rem;border:0;font-weight:500}
...
.svc-content-h,.fleet-text-title,.closer-h{font-family:var(–serif);font-size:clamp(1.8rem,3vw,2.6rem);font-weight:300;line-height:1.15;margin-bottom:1rem}
...
.why-n{color:var(–gold);font-size:.7rem}
.why-t{font-family:var(–serif);color:#fff;margin:.7rem 0;font-weight:300}
.why-d{color:rgba(255,255,255,.4);font-size:.85rem;line-height:1.7}

/* ── AREAS ── */
.areas-list{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid #eee}
.area-item{padding:2rem 1rem;border-bottom:1px solid #eee;cursor:pointer}
.area-name{font-family:var(–serif);margin-bottom:.4rem;font-weight:300}
...
.fleet-img-wrap,.moments-img-wrap{background:#1a1a1a;min-height:340px;overflow:hidden}
.fleet-img{width:100%;min-height:340px;object-fit:cover}
.fleet-text-eyebrow{color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.12em;font-size:.7rem}
.fleet-text-title,.fleet-text-sub,.fleet-text-body{color:#fff}
.fleet-text-sub,.fleet-text-body{color:rgba(255,255,255,.45);line-height:1.75}
...
.proc-name{font-family:var(–serif);color:#fff;margin:1rem 0;font-weight:300}
.proc-desc{color:rgba(255,255,255,.4);font-size:.85rem;line-height:1.7}
...
.moments-title{font-family:var(–serif);color:#fff;font-size:clamp(2rem,3vw,3rem);font-weight:300;line-height:1.2}
.moments-title em{color:rgba(255,255,255,.45)}
.moments-rule{width:2.5rem;height:2px;background:var(–gold);margin:1.5rem 0}
.moments-desc{color:rgba(255,255,255,.45);line-height:1.75}

/* ── TESTIMONIALS ── */
.testi-row{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:#eee}
.testi{background:#fff;padding:2.5rem}
.testi-mark{font-family:var(–serif);font-size:2rem;color:var(–gold)}
.testi-txt{font-family:var(–serif);font-style:italic;line-height:1.75;font-weight:300}
.testi-by{font-size:.7rem;color:#999;text-transform:uppercase;letter-spacing:.08em}

/* ── CLOSER ── */
.closer{background:#111;color:#fff;padding:10rem 5vw;text-align:center}
.closer-inner{max-width:640px;margin:auto}
.closer-sub,.closer-note{color:rgba(255,255,255,.35)}
...
/* ── FLOAT ── */
.wa-float{position:fixed;right:2rem;bottom:2rem;background:var(–wa);color:#fff;padding:.8rem 1.3rem;z-index:999;display:flex;gap:.6rem;align-items:center;font-size:.8rem;text-transform:uppercase;letter-spacing:.06em;font-weight:500}
...
.hero-content{padding:100px 5vw 60px}
.hero-h1{font-size:2.2rem}
.hero-services{flex-direction:column;gap:1px}
