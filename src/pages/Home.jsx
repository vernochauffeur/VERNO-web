...
function buildWhatsAppLink({ from, to, date, time, pax, bags, fare, flightNumber }) { const msg = ["Hello, I'd like to book a transfer:", "", `Pickup: ${from || "--"}`, `Drop-off: ${to || "--"}`, ...(date ? [`Date: ${date}`] : []), ...(time ? [`Time: ${time}`] : []), ...(pax ? [`Passengers: ${pax}`] : []), ...(bags ? [`Luggage: ${bags}`] : []), ...(fare ? [`Estimated fare: $${fare}`] : []), "", "Please confirm availability."].join("\n"); return `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(msg)}`; }
